...
// ThreadMap represents a collection of threads.
// The structure maps thread ID to thread struct.
type ThreadMap map[int]Thread

// Each will apply the function f to each thread, returning
// early if f returns an error.
func (tm ThreadMap) Each(f func(Thread) error) error {
	for _, th := range tm {
		if err := f(th); err != nil {
			return err
		}
...
type Thread interface {
	...
	WriteMemory(uintptr, []byte) (int, error)
	ReadMemory(uintptr, int) ([]byte, error)

	Registers() (Registers, error)
	SetRegisters(Registers) error
}

// BasicThread represents a single thread in the traced process
