...
func continueThread(t Thread) error {
	return continueThreadWithSignal(t.ID(), 0)
}

func continueThreadWithSignal(tid, sig int) error {
	var err error
	onPtraceThread(func() { err = PtraceCont(tid, sig) })
	return err
}
