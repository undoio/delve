...
// It also handles functionality needed by clients, but not needed in
...
func New() *Debugger {
	return &Debugger{processMutex: &sync.Mutex{}}
}

func (d *Debugger) AttachProcess(pid int) error {
	log.Printf("attaching to pid %d", pid)
	p, err := proc.Attach(pid)
	if err != nil {
		return attachErrorMessage(pid, err)
	}
	d.process = p
	return nil
}

func (d *Debugger) StartProcess(cmd []string) error {
	log.Printf("launching process with args: %v", cmd)
...
// 	// access the process directly, and do it before we acquire a mutex.
...
// 			return nil, err
// 		}
// 		ws, err := d.process.Wait()
// 		if err != nil {
// 			log.Printf("error during wait: %v", err)
// 			return nil, err
// 		}
// 		if ws.Exited() {
// 			if err := d.process.Mourn(); err != nil {
// 				log.Printf("error mourning: %v", err)
// 				return nil, err
// 			}
// 			return &api.DebuggerState{
// 				Exited:     true,
// 				ExitStatus: ws.ExitStatus,
// 				Err:        errors.New("process has exited"),
// 			}, nil
// 		}
