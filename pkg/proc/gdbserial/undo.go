...
func undoGetLogExtent(conn *gdbConn) (uint64, uint64, error) {
	...
	// The server terminates every number with a comma, so trim the trailing
	// one to avoid a spurious empty field.
	bbcounts := strings.Split(strings.TrimSuffix(extent, ","), ",")
